...
void indication (INDICATION_TYPE type, TRANS_STATUS info)
{
  ...
  if (type == IND_MACHINE_ALLOCATED)
	...
	sprintf (substring, "sending '%s' -> '%s'",
		 info.md.source_file_name, info.md.dest_file_name);
      else if ((info.role == CLASS_1_RECEIVER) ||
	      (info.role == CLASS_2_RECEIVER))
	{
	  if (info.has_md_been_received)
	    sprintf (substring, "receiving '%s' <- '%s'",
		     info.md.dest_file_name, info.md.source_file_name);
	  else
...
#define  MAX_USER_INPUT_LENGTH  128
{
  ...
  if (fgets (user_input, MAX_USER_INPUT_LENGTH, fp) == NULL)
      ...
      if (fp == stdin)
	{
	  if (feof (stdin))
	    {
	      printf ("^^^ EOF from stdin.\n");
	      strcpy (user_input, " ");
	      return;
	    }
	  else
	    {
	      printf ("^^^ fgets failed (stdin).\n");
	      printf ("^^^ Bye.\n");
	      exit (1);
	    }
	}
      ...
      else  /* named-pipe fgets failure */
	{
	  if (feof (fp))
	    {
	      printf ("^^^ EOF from named-pipe.\n");
	      strcpy (user_input, " ");
	      return;
	    }
	  else
	    {
	      printf ("^^^ fgets failed (named-pipe).\n");
	      printf ("^^^ Bye.\n");
	      exit (1);
	    }
	}
  ...
  if (strlen(user_input) == 0)
	...
	{
	  system ("date");
	  printf ("^^^ m__read_user_input:  string length is zero!\n");
	  fflush (stdout);
	}
      return;
    }
...
boolean user_input__poll (char *user_input)
   ...
   /*------------------------------------------------------------*/
	 ...
	 if (!eof_from_pipe)
	   {
		 ...
		 if (m_debug_pipe)
		   {
		     printf ("^^^ Got message from tester.\n");
		     fflush (stdout);
		   }
		 ...
		 if (m_debug_pipe)
		   {
		     printf ("^^^ Ack'd message from tester.\n");
		     fflush (stdout);
		   }
     ...
     if (eof_from_stdin && eof_from_pipe)
       {
	 printf ("^^^ user_input__poll:  Both sources have given EOF.\n");
	 printf ("Bye.\n");
	 exit (1);
       }
