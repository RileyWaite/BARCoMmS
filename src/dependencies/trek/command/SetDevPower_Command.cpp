...
SetDevPower_Command::SetDevPower_Command()
{
    APID = 16;
}

bool
SetDevPower_Command::fromBytes(ByteBuffer& buf)
{
   ...
   buf.getBits(160, 8, device);
   buf.getBits(168, 8, state);
...
bool
SetDevPower_Command::toBytes(ByteBuffer& buf)
{
   ...
   buf.putBits(160, 8, device);
   buf.putBits(168, 8, state);
